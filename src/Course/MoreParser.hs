...
tok p = do
  ...
  spaces
  return a
-- tok p = p >>= \a -> spaces >>= \_ -> return a
-- tok p = p >>= \a -> spaces >>= \_ -> valueParser a
-- tok p = (\a -> (\b -> valueParser a) =<< spaces) =<< p
...
-- | Write a function that tries the given parser. In case of error, succeeds by producing the given value.
--
-- /Tip:/ Use `|||`.
--
...
-- Iurii: Why `charTok`, not `character`?
--
-- to contribute
...
betweenCharTok ::
  ...
  -> Parser a
  -> Parser a
betweenCharTok b e = between (charTok b) (charTok e)
...
-- to contribute
-- My solution after using tips
hex :: Parser Char
hex = do
  hs <- replicateA 4 (satisfy isHexDigit)
  let Full r = readHex hs
  return $ chr r

-- TM's solution -- doesn't use the monad instance
-- hex :: Parser Char
-- hex =
...
-- hex :: Parser Char
-- hex = do
--   hs <- thisMany 4 (oneof "0123456789abcdefABCDEF")
--   let Full r = read $ "0x" ++ hs
--   return $ chr r
...
sepby1 p s = do
  start <- p
  rest  <- list (s >>> p) {- as an option could use `right apply (*>) instead of (>>>)`-}
  return (start :. rest)
...
-- eof = P (\i -> if isEmpty i then Result i () else ErrorResult Failed)
...
-- after being tipped (guessed by type signatures only):
satisfyAll ps = satisfy $ and . sequence ps
-- ps = (isUpper :. (/= 'X') :. Nil)
-- :t ps => List (Char -> Bool)
-- :t sequence ps => Char -> List Bool
-- :t and . sequence ps => Char -> Bool
-- :t satisfy $ and . sequence ps => Parser Char

-- solution w/o tip:
-- satisfyAll ps = satisfy (\c -> all ($ c) ps)
-- satisfyAll ps = satisfy (\c -> all (\p -> p c) ps)
...
-- satisfyAny ps = satisfy (\c -> any ($ c) ps)
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma b e p = betweenCharTok b e (sepby (tok p) commaTok)
